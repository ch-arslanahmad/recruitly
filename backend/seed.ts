...
db.prepare('INSERT INTO job (recruiter_id, title, description, location, salary, type) VALUES (?, ?, ?, ?, ?, ?)').run(1, 'Software Engineer', 'Build scalable backend services using Node.js and Python.', 'Lahore', 150000, 'full-time');
db.prepare('INSERT INTO job (recruiter_id, title, description, location, salary, type) VALUES (?, ?, ?, ?, ?, ?)').run(1, 'Frontend Developer', 'Create beautiful React UIs with Tailwind CSS.', 'Remote', 120000, 'remote');
db.prepare('INSERT INTO job (recruiter_id, title, description, location, salary, type) VALUES (?, ?, ?, ?, ?, ?)').run(2, 'Data Scientist', 'Analyze large datasets and build ML models.', 'Karachi', 180000, 'full-time');
db.prepare('INSERT INTO job (recruiter_id, title, description, location, salary, type) VALUES (?, ?, ?, ?, ?, ?)').run(2, 'DevOps Engineer', 'Manage cloud infrastructure on Azure.', 'Islamabad', 160000, 'contract');
db.prepare('INSERT INTO job (recruiter_id, title, description, location, salary, type) VALUES (?, ?, ?, ?, ?, ?)').run(1, 'Intern - Web Development', 'Learn web development with experienced mentors.', 'Lahore', 30000, 'part-time');
