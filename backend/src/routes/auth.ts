import { Request, Response, Router } from "express"; // map functions to endpoints
...
function register(req: Request, res: Response) {
    try {
        ...
        newUser.id = result.lastInsertRowid as number;
...
function login(req: Request, res: Response) {
    try {
        const oldUser: User | undefined = User.findByUsername(
            req.body?.username,
        );
