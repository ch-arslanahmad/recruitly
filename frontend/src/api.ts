...
export const api = {
  ...
  saved: {
    check: (id: string) =>
      fetch(`/api/saved-jobs/check/${id}`, { headers: authHeaders() }).then(
        (r) => {
          if (!r.ok) throw new Error("Failed to check saved job");
          return r.json();
        }
      ),
    ...
    toggle: async (jobId: number, isSaved: boolean) => {
      ...
      return fetch(url, {
        ...
        body: method === "POST" ? JSON.stringify({ job_id: jobId }) : undefined,
      }).then((r) => {
        if (!r.ok) throw new Error("Failed to toggle save");
      });
    },
  },
};
